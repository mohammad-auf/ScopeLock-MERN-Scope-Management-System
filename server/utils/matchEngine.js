/**
 * matchEngine.js — Scope match classifier (NFR6: isolated module)
 *
...
const classify = (requestText, categoryTag, scopeItems) => {
  ...
  // PRD: "A request is classified in_scope when its category tag equals the
  //       category tag of an existing scope item."
  ...
  // PRD: "A request is classified possible_extra when it has no category tag
  //       and shares no word of 4 or more letters, excluding a fixed stop
  //       word list, with any scope item title or description."
  // PRD: "A request is classified unclear when … it has no category tag but
  //       shares at least one word of 4 or more letters with a scope item
